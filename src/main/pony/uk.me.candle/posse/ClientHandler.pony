...
class ClientHandler is TCPConnectionNotify
	...
	fun sent(conn: TCPConnection ref, data: (String val | Array[U8 val] val))
		...
		out.print("<<< " + IPAddrString(conn.remote_address()) + " <<< " + output)
		data
	...
	fun received(conn: TCPConnection ref, data: Array[U8 val] iso): Bool =>
		let reader = Reader
		...
		while true do
			try
				...
				out.print(">>> " + IPAddrString(conn.remote_address()) + " >>> " + input)

				match user
				| let u: User => u.from_client(Message.from_raw(input))
				end
			else
